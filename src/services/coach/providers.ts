...
/** 응답 진행 상황을 UI에 전달하기 위한 훅 (스트리밍 토큰, 상태 메시지) */
...
/** 규칙 기반 분석 — AI 미연결 시 기본 피드백 */
export function buildMockAnalysis(ctx: CoachContext): string {
  ...
  lines.push('')
  lines.push('---')
  lines.push('*규칙 기반 엔진입니다. 설정에서 온디바이스 AI나 외부 API를 연결하면 대화형 코칭을 받을 수 있습니다.*')

  return lines.join('\n')
}

export async function chatCompleteOpenAI(
  apiKey: string,
  system: string,
...
export async function chatCompleteClaude(
  apiKey: string,
  system: string,
  messages: ChatMessage[],
): Promise<string> {
